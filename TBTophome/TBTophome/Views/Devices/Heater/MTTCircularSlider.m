...
- (void)setup
{
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(sliderChange:) name:SliderChange object:nil];
...
- (void)sliderChange:(NSDictionary *)dict
{
    NSDictionary *objectDict = [dict valueForKey:@"object"];
    NSInteger setTemp = [[objectDict valueForKey:@"setTemp"] integerValue];
    
//    _rotation += M_PI * 2 / 3;
    CGFloat rotation = (((setTemp - 25.0) / 50.0) * 5.0 / 3.0) * M_PI;
//    [self setNeedsDisplay];
    CGAffineTransform transform = CGAffineTransformMakeRotation((M_PI * _minAngle) / 180.0);
    _currentTransform = transform;
    transform = CGAffineTransformRotate(_currentTransform, rotation);
...
{
    [super drawRect:rect];
        _minRotation = M_PI * 2 / 3 - 0.1;
        _rotation += M_PI * 2 / 3 + 0.05;
        CGFloat centerX = self.frame.size.width / 2.f;
        CGFloat centerY = self.frame.size.height / 2.f;
        CGFloat radius = 105.f / 234.f * self.frame.size.width ;
        CGRect imageRect = CGRectMake(12.f / 234.f * self.frame.size.width, 12.f / 234.f * self.frame.size.width, 210.f / 234.f * self.frame.size.width, 210.f / 234.f * self.frame.size.width);
        CGRect indicatorRect = CGRectMake(centerX + (radius - 6.f/ 234.f * self.frame.size.width) * cos(_rotation) - 18.f/ 234.f * self.frame.size.width, centerY + (radius - 6.f/ 234.f * self.frame.size.width) * sin(_rotation) - 18, 35.f / 234.f * self.frame.size.width, 35.f / 234.f * self.frame.size.width);
        ...
        CGContextAddArc(context, centerX, centerY, radius, _minRotation, _rotation, 0);
...
//        CGContextTranslateCTM(context, -(centerX), -(centerY));
        CGContextDrawImage(context, indicatorRect, self.indicatorImage.CGImage);
}
