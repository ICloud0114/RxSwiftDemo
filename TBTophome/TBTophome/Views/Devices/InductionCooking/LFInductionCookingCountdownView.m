...
@interface LFInductionCookingCountdownView () <UIPickerViewDataSource, UIPickerViewDelegate>
...
@implementation LFInductionCookingCountdownView
...
- (IBAction)sure:(id)sender {
    [self dismiss];
    if ([self.delegate respondsToSelector:@selector(didSelectedCountdownTime:)]) {
        [self.delegate didSelectedCountdownTime:[self.pickerView selectedRowInComponent:0] + 1];
    }
}
...
+ (instancetype)inductionCookingCountdownView {
    UINib *nib = [UINib nibWithNibName:@"LFInductionCookingCountdownView" bundle:nil];
